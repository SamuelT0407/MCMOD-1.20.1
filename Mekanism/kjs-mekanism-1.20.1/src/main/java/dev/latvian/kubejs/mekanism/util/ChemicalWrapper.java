...
// @formatter:on
	...
	public record OutputComponent<C extends Chemical<C>, S extends ChemicalStack<C>, I extends ChemicalStackIngredient<C, S>>(
		...
		public S read(RecipeJS recipe, Object from) {
			if (from == null) {
				throw new RuntimeException("Chemical output cannot be null! Expected " + wrapper.key() + " chemical stack.");
			}
			if (wrapper.stackType.isInstance(from)) {
				return wrapper.stackType.cast(from);
			} else if (wrapper.ingredientType.isInstance(from)) {
				return wrapper.stackType.cast(from);
			} else if (from instanceof CharSequence) {
				var amount = wrapper.defaultAmount;
				var string = from.toString();
			...
			throw new RuntimeException("Cannot read " + wrapper.key() + " chemical output from: " + from + " (type: " + from.getClass().getName() + ")");
		}
	}
	...
	public static GasStack ofGasStack(Object o) {
		if (o instanceof GasStack gas) {
			return gas;
		} else if (o instanceof CharSequence) {
			long amount = GAS.defaultAmount();
			String gas = o.toString();
			...
			if (map != null) {
				...
				var amount = map.containsKey(JsonConstants.AMOUNT) ? ((Number) map.get(JsonConstants.AMOUNT)).longValue() : GAS.defaultAmount();
				if (id != null) {
					return GAS.stack(id.toString(), amount);
				}
			}
		}
